    ...
    <div className="flex flex-col h-full" data-testid="alerts-panel">
      <div className="flex items-center justify-between gap-1 mb-2">
        ...
        {activeAlerts.length > 0 && (
          <Badge variant="destructive" className="text-[11px] animate-pulse-glow">
            {t("alerts.active", { count: activeAlerts.length })}
          </Badge>
        )}
      </div>
        ...
        <div className="space-y-1.5 pr-1" aria-live="assertive">
          {activeAlerts.length === 0 && recentInactive.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-6">
              ...
              <p className="text-[11px] text-emerald-400 font-semibold">{t("alerts.noAlerts")}</p>
              <p className="text-[11px] text-muted-foreground">{t("alerts.allClear")}</p>
            </div>
          ) : (
            <>
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-red-200/70">{alert.threat}</p>
                      <div className="flex items-center gap-1 mt-1">
                        <MapPin className="w-2.5 h-2.5 text-red-400/50" />
                        <span className="text-[11px] text-red-400/50 tabular-nums">
              ...
              {recentInactive.length > 0 && (
                <div className="mt-2">
                  <p className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mb-1.5 font-semibold">
                    {t("alerts.recentlyExpired")}
                  </p>
                  {recentInactive.map((alert) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="text-[11px] text-muted-foreground">{alert.area}</span>
